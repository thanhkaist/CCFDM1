...
class ReplayBufferMultiTasks(Dataset):
    ...
    def __init__(self, obs_shape, action_shape, task_shape, capacity, batch_size, device, image_size=84,
                 transform=None):
        self.capacity = capacity
        self.batch_size = batch_size
        self.device = device
        ...
        self.rewards = np.empty((capacity, 1), dtype=np.float32)
        self.not_dones = np.empty((capacity, 1), dtype=np.float32)
        self.task_desc = np.empty((capacity, *task_shape), dtype=np.float32)

        self.idx = 0
        self.last_save = 0
        self.full = False

    def padding(self, action):
        if action.shape[0] < self.actions.shape[1]:
            padded_action = np.zeros(self.actions.shape[1], dtype=np.float32)
            padded_action[:action.shape[0]] = action
        else:
            padded_action = action
        return padded_action

    def add(self, obs, action, reward, next_obs, done, task):
        np.copyto(self.obses[self.idx], obs)
        save_action = self.padding(action)
        np.copyto(self.actions[self.idx], save_action)
        np.copyto(self.rewards[self.idx], reward)
        np.copyto(self.next_obses[self.idx], next_obs)
        np.copyto(self.not_dones[self.idx], not done)
        np.copyto(self.task_desc[self.idx], task)
